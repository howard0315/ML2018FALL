...
def SplitSentence():
	...
	for ind in TrainX.index:
		...
		WordList = [ch for wd in WordList 
			if wd not in StopWord if not re.match(r"B[0-9]+", wd) for ch in wd]
...
if __name__ == '__main__':
	...
	with open('./WordEmbedding/embedding_matrix_SC.pickle', 'rb') as file:
		embedding_matrix = pickle.load(file)
	
	with open('./WordEmbedding/word2idx_SC.pickle', 'rb') as file:
	...
	csv_logger = CSVLogger('./model/log/training_RNN_SingleChar.log')
	checkpoint = ModelCheckpoint(filepath='./model/best_RNN_SingleChar.h5',
	...
	model.save('./model/final_RNN_SingleChar.h5')
